...
export const WebICContextProvider = ({
  children,
  containerId,
  myUserId,
}: {
  ...
  const convertTreeToFileSystem = useCallback(
    (node: FileTreeResponse): FileSystemItem => {
      return {
        id: `file-${node.id}`,
        serverId: node.id,
        name: node.name,
        ...
        isOpen: false,
        children:
          node.children?.map((child) => convertTreeToFileSystem(child)) ||
  ...
  const refreshFileTree = useCallback(async () => {
    if (!containerId || isNaN(containerId)) {
      console.warn(
        "⚠️ containerId가 유효하지 않아 파일 트리를 로드할 수 없습니다. (containerId:",
        containerId,
        ")"
      );
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        const fileSystemItems = response.data.map((node) =>
          convertTreeToFileSystem(node)
        );
    ...
    const fetchStats = async () => {
      try {
        ...
        if (res?.ok) {
          ...
          const todayStat = weeklyData.daily?.find(
            (d: DailyStat) => d.todayDate === today
          );
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStats((prev) => {
          ...
          if (idx >= 0)
            newDaily[idx] = { ...newDaily[idx], codingTimeMs: totalMs };
  ...
  const saveFileContent = useCallback(
    async (content?: string) => {
      ...
      const contentToSave =
        content !== undefined ? content : activeFile.content;
      ...
      try {
        ...
        console.log("📤 API 요청");
        const token = await getToken();
        const response = await fileApi.updateFile(
          item.serverId,
          request,
          token || undefined
        );
  ...
  const deleteItem = useCallback(
    async (itemId: string) => {
      ...
      try {
        ...
        const token = await getToken();
        const response = await fileApi.removeFile(
          item.serverId,
          containerId || 0,
          token || undefined
        );
        ...
        setFiles((prev) => {
          const del = (items: FileSystemItem[]): FileSystemItem[] =>
            items
              ...
              .map((i) =>
                i.children ? { ...i, children: del(i.children) } : i
              );
          return del(prev);
        });
  ...
  const renameItem = useCallback(
    async (itemId: string, newName: string) => {
      ...
      if (!item || !item.serverId) {
        console.warn(
          "이름을 변경할 항목을 찾을 수 없거나 serverId가 없습니다."
        );
        return;
      }
  ...
  const moveItem = useCallback(
    async (itemId: string, targetParentId?: string) => {
      const item = findItem(files, itemId);
      if (!item || !item.serverId) {
  ...
  const loadFileContent = useCallback(
    async (fileId: string) => {
      ...
      try {
        ...
        console.log("📤 API 요청");
        const token = await getToken();
        const response = await fileApi.getFileContent(
          item.serverId,
          token || undefined
        );
        console.log("✅ API 응답:", response.data);
